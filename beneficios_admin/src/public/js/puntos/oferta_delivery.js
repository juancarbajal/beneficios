...
function removedivrecovered(e) {
    ...
    if (r == true) {
        ...
        if (hval !== 0) {
            $.ajax({
                url: "/delivery-puntos/deleteData",
                type: "POST",
                data: {id: hval},
                dataType: "json"
            })
                .done(function (data, textStatus, jqXHR) {
                    if (console && console.log) {
                        if (data.response == true) {
                            ...
                            content.find('p.error').html(data.message);
                        }
                    }
                })
                .fail(function (jqXHR, textStatus, errorThrown) {
...
function createblocksrecover(id, value) {
    ...
    var nombre = '<div class="col-md-1"><label>Nombre:</label></div>' +
        '<div class="col-md-3">' +
        '<label>' +
        '<input type="text" ' + nombrevalue + ' id="nombre_' + id + '" class="form-control name-control" name="nombre[' + id + ']">' +
        '</label>' +
        '<ul style="display:none;color:red;"> ' +
        '<li>El Nombre es requerido y no puede quedar vacío.</li>' +
        '</ul>' +
    ...
    var contenido = '<div class="col-md-12 children"><hr>' +
        ...
        nombre + detalle + boton +
        '</div>' +
        '<div class="col-md-12"><p class="error"></p></div>' +
        '</div>';
