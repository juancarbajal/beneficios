...
$(document).ready(function () {
    ...
    $("#finalizarEliminado").click(function () {
        ...
        if (tipo == "Campaña") {
            url = "/campanias-premios/delete";
            retornar = "/campanias-premios";
        } else if (tipo == "Segmento") {
            var categoria = $('input:hidden[name=campania]').val();
            url = "/campanias-premios/deleteSegmento";
            retornar = "/campanias-premios/edit/" + categoria;
        }
...
$(document).on('change', 'input.classicPers, input.classicPtos', function () {
    ...
    var premios = $(contenedor).find('input.classicPtos').val();
    var personas = $(contenedor).find('input.classicPers').val();
    var subtotal = Number(premios * personas);
...
function classicFields() {
    ...
    var contenido = '<div>' +
        ...
        '<div class="col-md-2"><input title="premios por usuario" class="form-control classicPtos" name="classicPtos[' + count + ']" type="text">' +
