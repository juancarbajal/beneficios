...
class DemandaPremiosEmpresasAdicionalesTable
{
    ...
    {
        $id = (int)$id;
        $rowset = $this->tableGateway->select(array("BNF3_Demanda_id" => $id, "NombreEmpresa" => $adicional));
        $row = $rowset->current();
        if (!$row) {
            return 0;
        }
    ...
    public function saveDemandaEmpresasAdicionales(DemandaPremiosEmpresasAdicionales $demandaEmpresas)
    {
        $data = array(
    ...
    public function updateDemandaEmpresaAdicionales($id, $adicional)
    {
        $data["Eliminado"] = 0;
        $data['FechaActualizacion'] = date("Y-m-d H:i:s");
        $this->tableGateway->update($data, array('BNF3_Demanda_id' => $id, "NombreEmpresa" => $adicional));
    }
}
