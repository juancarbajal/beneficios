...
class DemandaPremiosSegmentosTable
{
    ...
    public function getIfExist($id, $departamento)
    {
        ...
        $rowset = $this->tableGateway->select(array('BNF3_Demanda_id' => $id, "BNF3_Segmento_id" => $departamento));
        $row = $rowset->current();
        if (!$row) {
            return 0;
        }
    ...
    public function saveDemandaSegmentos(DemandaPremiosSegmentos $demandaSegmentos)
    {
        $data = array(
            'BNF3_Segmento_id' => $demandaSegmentos->BNF3_Segmento_id,
            'BNF3_Demanda_id' => $demandaSegmentos->BNF3_Demanda_id,
    ...
    public function updateDemandaSegmentos($id, $segmento)
    {
        $data["Eliminado"] = 0;
        $data['FechaActualizacion'] = date("Y-m-d H:i:s");
        $this->tableGateway->update($data, array('BNF3_Demanda_id' => $id, "BNF3_Segmento_id" => $segmento));
    }
}
