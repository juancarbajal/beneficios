...
class DemandaRubrosTable
{
    ...
    public function getIfExist($id, $rubro)
    {
        $id = (int)$id;
        $rubro = (int)$rubro;
        $rowset = $this->tableGateway->select(array('BNF2_Demanda_id' => $id, "BNF_Rubro_id" => $rubro));
        $row = $rowset->current();
        if (!$row) {
            return 0;
        }
    ...
    public function saveDemandaRubro(DemandaRubros $demandaRubros)
    {
        $data = array(
            'BNF_Rubro_id' => $demandaRubros->BNF_Rubro_id,
            'BNF2_Demanda_id' => $demandaRubros->BNF2_Demanda_id,
    ...
    public function updateDemandaRubros($id, $rubro)
    {
        $data["Eliminado"] = 0;
        $data['FechaActualizacion'] = date("Y-m-d H:i:s");
        $this->tableGateway->update($data, array('BNF2_Demanda_id' => $id, "BNF_Rubro_id" => $rubro));
    }
}
