...
class DemandaPremiosDepartamentosTable
{
    ...
    public function getIfExist($id, $departamento)
    {
        ...
        $rowset = $this->tableGateway->select(array('BNF3_Demanda_id' => $id, "BNF_Departamentos_id" => $departamento));
        $row = $rowset->current();
        if (!$row) {
            return 0;
        }
    ...
    public function saveDemandaDepartamento(DemandaPremiosDepartamentos $demandaDepartamentos)
    {
        $data = array(
            'BNF_Departamentos_id' => $demandaDepartamentos->BNF_Departamentos_id,
            'BNF3_Demanda_id' => $demandaDepartamentos->BNF3_Demanda_id,
    ...
    public function updateDemandaDepartamentos($id, $departamento)
    {
        $data["Eliminado"] = 0;
        $data['FechaActualizacion'] = date("Y-m-d H:i:s");
        $this->tableGateway->update($data, array('BNF3_Demanda_id' => $id, "BNF_Departamentos_id" => $departamento));
    }
}
