...
use Demanda\Model\Filter\DemandaPremiosFilter;
use Zend\Json\Json;
...
class DemandaPremiosController extends AbstractActionController
{
    ...
    {
        return $this->serviceLocator->get('Demanda\Model\Table\DemandaPremiosTable');
    }

    public function getDemandaEmpresaTable()
    {
        return $this->serviceLocator->get('Demanda\Model\Table\DemandaPremiosEmpresasTable');
    }

    public function getDemandaDepartamentoTable()
    {
        return $this->serviceLocator->get('Demanda\Model\Table\DemandaPremiosDepartamentosTable');
    }

    public function getDemandaRubroTable()
    {
        return $this->serviceLocator->get('Demanda\Model\Table\DemandaPremiosRubrosTable');
    }

    public function getDemandaEmpresaAdicionalTable()
    {
        return $this->serviceLocator->get('Demanda\Model\Table\DemandaPremiosEmpresasAdicionalesTable');
    }

    public function getDemandaLogTable()
    {
        return $this->serviceLocator->get('Demanda\Model\Table\DemandaPremiosLogTable');
    }

    public function getDemandaSegmentosTable()
    {
        return $this->serviceLocator->get('Demanda\Model\Table\DemandaPremiosSegmentosTable');
    }
    ...
    public function indexAction()
    {
        ...
        if (array_key_exists($order_by, $busqueda)) {
            ...
            $order_by_o = 'id';
            $order_by = 'BNF3_Demanda.FechaCreacion';
        }
    ...
    public function addAction()
    {
        ...
        $request = $this->getRequest();
        if ($request->isPost()) {
            $validate = new DemandaPremiosFilter();
            ...
            if ($form->isValid() and empty($messages)) {
                ...
                foreach ($request->getPost()->Segmento as $value) {
                    $demandaSegmentos = new DemandaPremiosSegmentos();
                    $demandaSegmentos->BNF3_Segmento_id = $value;
                    $demandaSegmentos->BNF3_Demanda_id = $demanda->id;
                ...
                foreach ($request->getPost()->Departamentos as $value) {
                    $demandaDepartamento = new DemandaPremiosDepartamentos();
                    $demandaDepartamento->BNF_Departamentos_id = $value;
                    $demandaDepartamento->BNF3_Demanda_id = $demanda->id;
                ...
                foreach ($request->getPost()->Rubros as $value) {
                    $demandaRubro = new DemandaPremiosRubros();
                    $demandaRubro->BNF_Rubro_id = $value;
                    $demandaRubro->BNF3_Demanda_id = $demanda->id;
                ...
                foreach ($request->getPost()->EmpresaProveedor as $value) {
                    $demandaEmpProv = new DemandaPremiosEmpresas();
                    $demandaEmpProv->BNF_Empresa_id = $value;
                    $demandaEmpProv->BNF3_Demanda_id = $demanda->id;
                ...
                if (!empty($parts)) {
                    foreach ($parts as $value) {
                        $demandaEmpAdd = new DemandaPremiosEmpresasAdicionales();
                        $demandaEmpAdd->NombreEmpresa = $value;
                        $demandaEmpAdd->BNF3_Demanda_id = $demanda->id;
        ...
        try {
            ...
            foreach ($segmentos as $segmento) {
                ...
                $dataSegJS = $contador > 0 ?
                    $dataSegJS . ", '" . $segmento->BNF3_Segmento_id . "'" : "['" . $segmento->BNF3_Segmento_id . "'";
            ...
            //Campaña y Segmento
            $segmento = $this->getSegmentoTable()->getSegmentosPremios($dataSeg[0]);
            $campania = $segmento->BNF3_Campania_id;
        } catch (\Exception $ex) {
            ...
            if ($form->isValid() and empty($messages)) {
                ...
                foreach ($request->getPost()->Departamentos as $value) {
                    if ($this->getDemandaDepartamentoTable()->getIfExist($demanda->id, $value)) {
                        ...
                        $demandaSegmentos = new DemandaPremiosDepartamentos();
                ...
                #region Guardar Log
                $demandaLog = new DemandaPremiosLog();
                $demandaLog->BNF3_Demanda_id = $id;
    ...
    public function exportAction()
    {
        ...
        if ($registros > 0) {
            ...
            $objPHPExcel->getActiveSheet()->setAutoFilter('A1:D' . $registros);
            ...
            $objPHPExcel->getActiveSheet()->getStyle('A1:D' . ($registros + 1))->applyFromArray($styleArray2);
            $objPHPExcel->getActiveSheet()->getStyle('A1:D1')->applyFromArray($styleArray);
            $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A1', 'id')
                ->setCellValue('B1', 'Nombre de Campania')
                ->setCellValue('C1', 'Presupuesto')
                ->setCellValue('D1', 'VigenciaInicio');
            $i = 2;
            ...
            foreach ($resultado as $registro) {
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('D' . $i, $registro->FechaDemanda);
                $i++;
            }
        }
    ...
    public function enviarCorreo($request, $email)
    {
        ...
        $segmento = $this->getSegmentoTable()->getSegmentosPremios($request->getPost()->Segmento);
