...
class MetClienteTable
{
    ...
    {
        $sql_str = "TRUNCATE TABLE `BNF_DM_Met_Cliente`";
    ...
    {
        $sql_str = "INSERT INTO BNF_DM_Met_Cliente
                        SELECT
                        null,
                        CASE
                        ...
                        cu.BNF_Categoria_id,
                        r.id,
                        cu.BNF_ClienteCorreo_id,
                        CASE
                            WHEN p.Pregunta03 REGEXP '^-?[0-9]+$' AND p.Pregunta03 < YEAR(curdate()) THEN (YEAR(curdate()) - p.Pregunta03)
                            ELSE NULL
                        END Edad,
                        CASE
                        ...
                        END nivel_estudios
                    FROM BNF_Cupon cu
                    LEFT JOIN BNF_Cliente c ON c.id = cu.BNF_Cliente_id
                    INNER JOIN BNF_Oferta o ON o.id = cu.BNF_Oferta_id
                    INNER JOIN BNF_OfertaUbigeo ou ON ou.BNF_Oferta_id = o.id
                    INNER JOIN BNF_Ubigeo u ON u.id = ou.BNF_Ubigeo_id
                    LEFT JOIN BNF_Preguntas p ON p.BNF_Cliente_id = c.id
                    INNER JOIN BNF_OfertaRubro ofr ON ofr.BNF_Oferta_id = o.id
                    INNER JOIN BNF_Rubro r ON r.id = ofr.BNF_Rubro_id
                    WHERE FechaGenerado IS NOT NULL
                    AND cu.BNF_Empresa_id IS NOT NULL
                    GROUP BY cu.id";
        $statement = $this->adapter->createStatement($sql_str);
    ...
    public function envios()
    {
        $sql_str = "INSERT INTO BNF_DM_Met_Cliente
                    ...
                    FROM
                    BNF_Cliente c
                    INNER JOIN BNF_OfertaFormCliente cu ON c.id = cu.BNF_Cliente_id
                    INNER JOIN BNF_Oferta o ON o.id = cu.BNF_Oferta_id
                    INNER JOIN BNF_OfertaUbigeo ou ON ou.BNF_Oferta_id = o.id
                    INNER JOIN BNF_Ubigeo u ON u.id = ou.BNF_Ubigeo_id
                    LEFT JOIN BNF_Preguntas p ON p.BNF_Cliente_id = c.id
                    INNER JOIN BNF_OfertaRubro ofr ON ofr.BNF_Oferta_id = o.id
                    INNER JOIN BNF_Rubro r ON r.id = ofr.BNF_Rubro_id
                    WHERE cu.BNF_Empresa_id IS NOT NULL
                    GROUP BY cu.id";
        $statement = $this->adapter->createStatement($sql_str);
