...
use Puntos\Form\FormReporteProveedorOfertas;
use Zend\Json\Json;
    ...
    public function indexAction()
    {
        ...
        if ($request->isPost()) {
            ...
            if ($valid->getMessages()) {
                $valid = new RecordExists(
                    array(
                        'table' => 'BNF2_Oferta_Puntos_Atributos',
                        'field' => 'id',
                        'adapter' => $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter')
                    )
                );
            ...
            if (!$errors['empresa'] && !$errors['campania'] && !$errors['ofertas']) {
                ...
                $objPHPExcel->getProperties()
                    ...
                    ->setTitle("Reporte de Proveedor Ofertas")
                    ->setSubject("Reporte Proveedor Ofertas")
                    ->setDescription("Documento Reporte de Proveedor Ofertas")
                    ->setKeywords("Beneficios.pe")
                    ->setCategory("Reporte Proveedor Ofertas");
                ...
                if($split) {
                    $resultado = $this->getCuponPuntosTable()->getCuponesByAtributo((int)$data['ofertas']);
                    $atributo = $this->getOfertaPuntosAtributosTable()->getOfertaPuntosAtributos((int)$data['ofertas']);
                    $oferta = $atributo->BNF2_Oferta_Puntos_id;
                } else {
                    $resultado = $this->getCuponPuntosTable()->getCuponesForOferta((int)$data['ofertas']);
                    $oferta = $data['ofertas'];
                }
                ...
                $oferta = $this->getOfertaPuntosTable()->getOfertaPuntos((int)$oferta);
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A1', 'Reporte proveedor ofertas')
                    ->mergeCells('A1:B1')
                ...
                if ($registros > 0) {
                    ...
                    foreach ($resultado as $registro) {
                        ...
                        $logs = $this->getCuponPuntosLogTable()->getCuponPuntosLogByCupon($registro->id);
                ...
                header('Content-Disposition: attachment;filename="ReporteProveedorOfertas.xlsx"');
            ...
            if (isset($post_data['csrf'])) {
                if ((filter_var($id, FILTER_VALIDATE_INT) !== false) and $csrf->verifyToken($post_data['csrf'])
                    ...
                    if ($result = $this->getEmpresaTable()->getEmpresa($id)) {
                        $campanias = $this->getCampaniaPuntosTable()->getCampaniasPByEmpresaProv($id);
        ...
        if ($request->isPost()) {
            ...
            if (isset($post_data['csrf'])) {
                if ((filter_var($id, FILTER_VALIDATE_INT) !== false) and $csrf->verifyToken($post_data['csrf'])
                    ...
                    if ($result = $this->getCampaniaPuntosTable()->getIfExist($id)) {
                        $ofertas = $this->getOfertaPuntosTable()
                            ->getAllOfertaPuntosByCampaniaAndEmpresaProv($id, $empresa);
