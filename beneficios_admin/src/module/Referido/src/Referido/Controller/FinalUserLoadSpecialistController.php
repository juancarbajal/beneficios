...
class FinalUserLoadSpecialistController extends AbstractActionController
{
...
//    {
//        return $this->serviceLocator->get('Cliente\Model\BeneficiarioTable');
//    }
    ...
    public function loadSpecialistAction()
    {
        $fileMessage = '';
        $successMessage = '';
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                if (!$adapter->isValid() || !$adapter2->isValid()) {
                    ...
                    if ($highestColumm == "E") {
                        foreach ($objWorksheet->getRowIterator(1) as $row) {
                            ...
                            if ($rowIndex > 1) {
                                ...
                                if ($data_cell[4] == '') {
                                    $errorValid = true;
                                    $errorMessageCsv[$rowIndex] = $rowError . 'Falta ingresar el beneficiario.';
                                }
                                if ($data_cell[0] == '') {
                                    $errorValid = true;
                    ...
                    if (!$errorCsv && !$errorDni && !$errorValid && !$errorPasaporte) {
